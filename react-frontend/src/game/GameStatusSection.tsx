...
export default function GameStatusSection({ game }: Props) {
    ...
    const handleStartGame = () => {
        remoteService.post(`/api/game/${game.id.value}/start`).then(() => {
    ...
    return (
        <div>
            ...
            {game.status === GameStatus.IN_PROGRESS && <Section>Turn {game.state.moves.length + 1}</Section>}
            {game.status === GameStatus.DELETED && <Section>Deleted game</Section>}
